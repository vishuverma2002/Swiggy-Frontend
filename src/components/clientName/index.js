...
function Investor_Name() {
  ...
  useEffect(() => {
    ...
    if (token) {
      setClientName(parseJwt(token));
    } else {
      setClientName({ "clientName ": "Guest" });
    }
  }, [investorId]);
  ...
  return (
    ...
    >
      ...
      <span style={{ color: "#fff", display: "flex", fontSize: "13px", alignItems: "center", gap: 1 }}>
        {clientName && capitalizeEachWord(clientName?.["clientName "] || "Guest")} <IoMdArrowDropdown size={18} color="#ffffff" />
      </span>
    </div>
  );
}
